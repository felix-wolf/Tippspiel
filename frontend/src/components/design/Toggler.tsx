...
export function Toggler({
  ...
  return (<>
    <div className="w-full flex flex-row justify-center px-2">
      <div className="flex flex-wrap items-center justify-center bg-white/40 backdrop-blur-md border border-white/40 rounded-xl shadow-sm my-3">
        {items.map((item, index) => (
          <button
            className={`
            flex items-center gap-2 px-3 sm:px-5 py-2 text-sm sm:text-base font-medium transition rounded-xl
    ...
    {items.filter((item) => item.component != null).length != 0 && (
      <motion.section
        className="w-full max-w-6xl backdrop-blur-md bg-white/40 border border-white/40 rounded-3xl shadow-lg p-4 sm:p-6 mb-8"
        layout
      >
